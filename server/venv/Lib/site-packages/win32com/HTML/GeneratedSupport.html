...
<P><IMG ALT="Python and COM - Blowing the others away" HEIGHT=99 SRC="image/pycom_blowing.gif" WIDTH=549></P>
<H1>Generated Python COM Support</H1>
<P>This file describes how the Python COM extensions support "generated files". The information contained here is for
  expert Python users, and people who need to take advantage of the advanced features of the support. More general
  information is available in the <A HREF="QuickStartClientCom.html">Quick Start to Client Side COM</A> documentation.
</P>
<H2>Introduction</H2>
<P>Generated Python COM support means that a .py file exists behind a particular COM object. This .py file is created by
  a generation process from a COM type library.</P>
<P>This documentation talks about the process of the creation of the .py files.</P>
<H2>Design Goals</H2>
<P>The main design goal is that the Python programmer need not know much about the type library they wish to work with.
  They need not know the name of a specific Python module to use a type library. COM uses an IID, version and LCID to
  identify a type library. Therefore, the Python programmer only need know this information to obtain a Python
  module.</P>
...
<P>Using makepy is in many ways far simpler - you simply pick the type library and you are ready to go! The <A
    HREF="QuickStartClientCom.html">Quick Start to Client Side COM</A> documentation describes this process.</P>
<P>Often however, you will want to use code to ensure the type library has been processed. This document describes that
  process.</P>
<H2>Usage</H2>
<P>The win32com.client.gencache module implements all functionality. As described above, if you wish to generate support
  from code, you need to know the IID, version and LCID of the type library.</P>
<P>The following functions are defined. The best examples of their usage is probably in the Pythonwin OCX Demos, and the
  COM Test Suite (particularly testMSOffice.py)</P>
<P>Note that the gencache.py file supports being run from the command line, and provides some utilities for managing the
  cache. Run the file to see usage options.</P>
<H2>Using makepy to help with the runtime generation</H2>
<P>makepy supports a "-i" option, to print information about a type library. When you select a type library, makepy will
  print out 2 lines of code that you cant paste into your application. This will then allow your module to generate
  the makepy .py file at runtime, but will only take you a few seconds!</P>
...
<P>Given the IID, LCID and version information for a type library, check and if necessary generate, then import the
